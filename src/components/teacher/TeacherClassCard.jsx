...
const TeacherClassCard = ({ title, section, studentCount, bannerColor }) => {
    return (
        <div className="bg-examsy-surface rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
            {/* Header / Banner */}
            <div className={`h-28 p-6 flex flex-col justify-between relative overflow-hidden ${bannerColor || 'bg-examsy-primary'}`}>
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-10 -mt-10 blur-2xl" />

                <div className="flex justify-between items-start relative z-10">
                    <h3 className="text-white text-xl font-black hover:underline cursor-pointer truncate pr-4">
                        {title}
                    </h3>
                    <button className="text-white/80 hover:text-white p-1 hover:bg-white/10 rounded-full transition-colors">
            ...
            <div className="px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-end gap-3 text-examsy-muted">
                <button className="p-2 hover:bg-examsy-bg rounded-xl transition-colors" title="Performance">
                    <TrendingUp size={18} />
                </button>
                <button className="p-2 hover:bg-examsy-bg rounded-xl transition-colors" title="Files">
                    <FolderOpen size={18} />
                </button>
            </div>
        </div>
